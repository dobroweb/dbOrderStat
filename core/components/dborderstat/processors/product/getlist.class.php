...
class msProductGetListProcessor extends modObjectGetListProcessor {
	...
	public $defaultSortField = 'all_count';
	...
	public function prepareQueryBeforeCount(xPDOQuery $c) {
		...
		$c->groupby('`msOrderProduct`.`product_id`, `all_options`');
		...
		$c->select('IFNULL(`msOrderProduct`.`options`,\'[""]\') as `all_options`');
	...
	public function prepareRow(xPDOObject $object) {
		$fields = array_map('trim', explode(',', $this->modx->getOption('dbos_product_fields', null, '')));
		...
		if (!empty($options) && is_array($options)) {
			...
			foreach ($options as $k => $v) {
				...
				$data['option_'.$k] = !$v ? $this->modx->lexicon('dbos_product_nooption') : $v;
			}
